...
contract Game is Ownable {
    ...
    function claimThrone() external payable gameNotEnded nonReentrant {
        ...
        require(
            // @audit-finding: typo `==` is replaced by `!=`
        ...
        // @audif-finding: this takes a percentage of the sentAmount (`msg.value`) instead of the current `claimFee`
        currentPlatformFee = (sentAmount * platformFeePercentage) / 100;

        // Defensive check to ensure platformFee doesn't exceed available amount after previousKingPayout
        // @audit-info: `previousKingPayout` is always 0 for all the claims, thus this if branch never executes
